function varargout = GLW_grand_average(varargin)
% GLW_GRAND_AVERAGE MATLAB code for GLW_grand_average.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_grand_average_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_grand_average_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_grand_average is made visible.
function GLW_grand_average_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_grand_average (see VARARGIN)
% Choose default command line output for GLW_grand_average
...
%fill table with dataset_names
for i=1:length(datasets);
    table{i,1}=datasets(i).header.name;
end;
%fill table with weights
if isempty(configuration.parameters.dataset_weights);
    for i=1:length(datasets);
        table{i,2}=1;
    end;
else
    for i=1:length(configuration.parameters.dataset_weights);
        table{i,2}=configuration.parameters.dataset_weights(i);
    end;
end;
%update table
...
function varargout = GLW_grand_average_OutputFcn(hObject, eventdata, handles) 
...
for i=1:size(table,1);
    configuration.parameters.dataset_weights(i)=table{i,2};
end;
%!!!
%END
%!!!
