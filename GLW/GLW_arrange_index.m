function varargout = GLW_arrange_index(varargin)
% GLW_ARRANGE_INDEX MATLAB code for GLW_arrange_index.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_arrange_index_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_arrange_index_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_arrange_index is made visible.
function GLW_arrange_index_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_arrange_index (see VARARGIN)
% Choose default command line output for GLW_arrange_index
...
%index_idx
index_idx=configuration.parameters.index_idx;
set(handles.listbox2,'Userdata',index_idx);
...
%index_idx
index_idx=get(handles.listbox2,'Userdata');
st={};
if isempty(index_idx);
else
    for i=1:length(index_idx);
        st{i}=num2str(index_idx(i));
    end;
end;
...
function varargout = GLW_arrange_index_OutputFcn(hObject, eventdata, handles) 
...
%!!!!!!!!!!!!!!!!!!!!
index_idx=get(handles.listbox2,'Userdata');
configuration.parameters.index_idx=index_idx;
%!!!
%END
%!!!
...
%update index_idx
if insert_pos==1;
    index_idx=[selected_epochs index_idx];
end;
if insert_pos==length(index_idx);
    index_idx=[index_idx selected_epochs];
end;
if (insert_pos>1)&(insert_pos<length(index_idx));
    index_idx=[index_idx(1:insert_pos-1) selected_epochs index_idx(insert_pos:end)];
end;
set(handles.listbox2,'Userdata',index_idx);
update_listbox2(handles);
