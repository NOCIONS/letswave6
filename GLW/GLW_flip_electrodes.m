function varargout = GLW_flip_electrodes(varargin)
% GLW_FLIP_ELECTRODES MATLAB code for GLW_flip_electrodes.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_flip_electrodes_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_flip_electrodes_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_flip_electrodes is made visible.
function GLW_flip_electrodes_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_flip_electrodes (see VARARGIN)
% Choose default command line output for GLW_flip_electrodes
...
function varargout = GLW_flip_electrodes_OutputFcn(hObject, eventdata, handles) 
...
function auto_flip(handles);
data=get(handles.chan_table,'Data');
%chan_labels
for i=1:size(data,1);
   chan_labels{i}=data{i,1};
end;
%
chan_labels2=chan_labels;
for i=1:length(chan_labels);
    ...
    if isempty(tp);
        ...
        if mod(tp,2)==1;
            ...
            disp([st1 ' ' st2]);
            %does st2 exist?
            a=find(strcmpi(chan_labels,st2));
            if isempty(a);
            else
                chan_labels2{i}=st2;
            end;
...
%
for i=1:length(chan_labels2);
    data{i,2}=chan_labels2{i};
end;
