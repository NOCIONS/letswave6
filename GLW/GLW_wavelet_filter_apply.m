function varargout = GLW_wavelet_filter_apply(varargin)
% GLW_WAVELET_FILTER_APPLY MATLAB code for GLW_wavelet_filter_apply.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_wavelet_filter_apply_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_wavelet_filter_apply_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_wavelet_filter_apply is made visible.
function GLW_wavelet_filter_apply_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_wavelet_filter_apply (see VARARGIN)
% Choose default command line output for GLW_wavelet_filter_apply
...
%!!!!!!!!!!!!!!!!!!!!!!!!
%template_filename
set(handles.template_filename_text,'String',configuration.parameters.template_filename);
%channel labels
...
function varargout = GLW_wavelet_filter_apply_OutputFcn(hObject, eventdata, handles) 
...
%!!!!!!!!!!!!!!!!!!!!
configuration.parameters.template_filename=get(handles.template_filename_text,'String');
...
% --- Executes on button press in select_filter_template_btn.
function select_filter_template_btn_Callback(hObject, eventdata, handles)
% hObject    handle to select_filter_template_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
FilterSpec={'*.lw6;*.LW6'};
filename=uigetfile(FilterSpec);
set(handles.template_filename_text,'String',filename);
%load header
header=CLW_load_header(filename);
%find history > wavelet_filter_build
entry_idx=0;
for i=1:length(header.history);
    if strcmpi('LW_wavelet_filter_build',header.history(i).configuration.gui_info.function_name);
        entry_idx=i;
    end;
end;
if entry_idx==0;
    disp('No wavelet filter template found.');
else
    channel_label=header.history(entry_idx).configuration.parameters.selected_channel;
    st=get(handles.channels_listbox,'String');
    a=find(strcmpi(channel_label,st));
    if isempty(a);
    else
        set(handles.channels_listbox,'Value',a(1));
    end;
