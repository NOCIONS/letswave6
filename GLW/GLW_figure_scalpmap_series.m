function varargout = GLW_figure_scalpmap_series(varargin)
% GLW_FIGURE_SCALPMAP_SERIES MATLAB code for GLW_figure_scalpmap_series.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_figure_scalpmap_series_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_figure_scalpmap_series_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_figure_scalpmap_series is made visible.
function GLW_figure_scalpmap_series_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_figure_scalpmap_series (see VARARGIN)
% Choose default command line output for GLW_figure_scalpmap_series
...
function varargout = GLW_figure_scalpmap_series_OutputFcn(hObject, eventdata, handles) 
...
%xi
xstart=str2num(get(handles.x_start_edit,'String'));
xend=str2num(get(handles.x_end_edit,'String'));
mapcount=fix(str2num(get(handles.num_maps_edit,'String')));
...
for row_pos=1:size(table_data,1);
    ...
    for i=1:length(dxi);
        ...
        %headplot
        CLW_topoplot(dataset.header,dataset.data,epoch_pos,index_pos,dxi(i),dy,dz,'maplimits',[cmin cmax]);
        %title
...
function num_maps_edit_Callback(hObject, eventdata, handles)
% hObject    handle to num_maps_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function num_maps_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to num_maps_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
if ispc && isequal(get(hObject,'BackgroundColor'), get(0,'defaultUicontrolBackgroundColor'))
...
for row_pos=1:size(table_data,1);
    ...
    for i=1:length(dxi);
        ...
        subplot(size(table_data,1),length(dxi),((row_pos-1)*length(dxi))+i);
        %vector
        vector=squeeze(dataset.data(epoch_pos,:,index_pos,dz,dy,dxi(i)));
