function varargout = GLW_arrange_epochs(varargin)
% GLW_ARRANGE_EPOCHS MATLAB code for GLW_arrange_epochs.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_arrange_epochs_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_arrange_epochs_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_arrange_epochs is made visible.
function GLW_arrange_epochs_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_arrange_epochs (see VARARGIN)
% Choose default command line output for GLW_arrange_epochs
...
%epoch_idx
epoch_idx=configuration.parameters.epoch_idx;
set(handles.listbox2,'Userdata',epoch_idx);
...
%epoch_idx
epoch_idx=get(handles.listbox2,'Userdata');
st={};
...
function varargout = GLW_arrange_epochs_OutputFcn(hObject, eventdata, handles) 
...
%!!!!!!!!!!!!!!!!!!!!
epoch_idx=get(handles.listbox2,'Userdata');
configuration.parameters.epoch_idx=epoch_idx;
%!!!
%END
%!!!
...
%update epoch_idx
if insert_pos==1;
    epoch_idx=[selected_epochs epoch_idx];
end;
if insert_pos==length(epoch_idx);
    epoch_idx=[epoch_idx selected_epochs];
end;
if (insert_pos>1)&(insert_pos<length(epoch_idx));
    epoch_idx=[epoch_idx(1:insert_pos-1) selected_epochs epoch_idx(insert_pos:end)];
end;
set(handles.listbox2,'Userdata',epoch_idx);
update_listbox2(handles);
