function varargout = GLW_dipfit_fit_events(varargin)
% GLW_DIPFIT_FIT_EVENTS MATLAB code for GLW_dipfit_fit_events.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_dipfit_fit_events_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_dipfit_fit_events_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_dipfit_fit_events is made visible.
function GLW_dipfit_fit_events_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GLW_dipfit_fit_events (see VARARGIN)
% Choose default command line output for GLW_dipfit_fit_events
...
if isempty(st);
    disp('No events in the selected datasets. Exit.');
    return;
end;
st=sort(unique(st));
...
if isempty(configuration.parameters.event_code);
    ...
    b=[];
    for i=1:length(configuration.parameters.event_code);
...
function varargout = GLW_dipfit_fit_events_OutputFcn(hObject, eventdata, handles) 
...
st=st(get(handles.event_code_listbox,'Value'));
configuration.parameters.event_code=st;
%dipole_model
