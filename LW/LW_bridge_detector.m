function [out_configuration,out_datasets] = LW_bridge_detector(operation,configuration,datasets,update_pointers)
% LW_bridge_detector
% 
%
% operations : 
...
%gui_info
gui_info.function_name='LW_bridge_detector';
gui_info.name='Electrode bridge detector';
gui_info.description='Electrode bridge detector';
...
gui_info.process_filename_string='bridge_detect';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_bridge_detector('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
