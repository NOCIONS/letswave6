function [out_configuration,out_datasets] = LW_arrange_index(operation,configuration,datasets,update_pointers)
% LW_arrange_index
% Rearrange / Delete index
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_arrange_index';
gui_info.name='Rearrange / Delete index';
gui_info.description='Rearrange / Delete index';
...
gui_info.process_filename_string='ep-index';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.index_idx=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_arrange_index(datasets(setpos).header,datasets(setpos).data,configuration.parameters.index_idx);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_arrange_index('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
