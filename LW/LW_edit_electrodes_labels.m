function [out_configuration,out_datasets] = LW_edit_electrodes_labels(operation,configuration,datasets,update_pointers)
% LW_edit_electrodes_labels
...
%gui_info
gui_info.function_name='LW_edit_electrodes_labels';
gui_info.name='Edit electrode labels';
gui_info.description='Edit electrode labels.';
...
gui_info.process_filename_string='chanlabels';  %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_edit_electrodes_labels(datasets(setpos).header,configuration.parameters.chan_labels);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_edit_electrodes_labels('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
