function [out_configuration,out_datasets] = LW_pool_channels2(operation,configuration,datasets,update_pointers)
% LW_pool channels
% pool channels
%
% operations : 
...
%gui_info
gui_info.function_name='LW_pool_channels2';
gui_info.name='Pool channels';
...
switch operation
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_pool_channels(datasets(setpos).header,datasets(setpos).data,configuration.parameters.channel_labels,'mixed_channel_label',configuration.parameters.mixed_channel_label,'keep_original_channels',configuration.parameters.keep_original_channels);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_pool_channels2('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
