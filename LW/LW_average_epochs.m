function [out_configuration,out_datasets] = LW_average_epochs(operation,configuration,datasets,update_pointers)
% LW_average_epochs
% Average epochs
%
% operations : 
...
%gui_info
gui_info.function_name='LW_average_epochs';
gui_info.name='Average epochs';
gui_info.description='Average epochs across trials.';
...
gui_info.process_filename_string='avg';         %default filename suffix
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Average epochs.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            ...
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_average_epochs(datasets(setpos).header,datasets(setpos).data,'operation',configuration.parameters.operation);
            %message_string
