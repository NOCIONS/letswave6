function [out_configuration,out_datasets] = LW_STFFT(operation,configuration,datasets,update_pointers)
% LW_STFFT
% Short-term FFT
%
% operations : 
...
%gui_info
gui_info.function_name='LW_STFFT';
gui_info.name='Short-term FFT (STFFT)';
gui_info.description='Compute the windowed Fourier transform (STFFT).';
...
gui_info.process_filename_string='stfft';     %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.sliding_step=1;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** ST-FFT.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_STFFT(datasets(setpos).header,datasets(setpos).data,'hanning_width',configuration.parameters.hanning_width,'sliding_step',configuration.parameters.sliding_step,'low_frequency',configuration.parameters.low_frequency,'high_frequency',configuration.parameters.high_frequency,'num_frequency_lines',configuration.parameters.num_frequency_lines,'postprocess',configuration.parameters.postprocess,'average_epochs',configuration.parameters.average_epochs,'segment_data',configuration.parameters.segment_data,'event_name',configuration.parameters.event_name,'x_start',configuration.parameters.x_start,'x_end',configuration.parameters.x_end);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_STFFT('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
