function [out_configuration,out_datasets] = LW_pool_channels(operation,configuration,datasets,update_pointers)
% LW_pool channels
% pool channels
%
% operations : 
...
%gui_info
gui_info.function_name='LW_pool_channels';
gui_info.name='New channel : linear mix';
gui_info.description='Compute new channel as linear mixture of existing channels.';
...
gui_info.process_filename_string='avgchan';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.channel_labels={};
        out_configuration.parameters.channel_weights=[1];
        out_configuration.parameters.mixed_channel_label='newchan';
        out_configuration.parameters.keep_original_channels=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Pool channels (weighted).',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_pool_channels(datasets(setpos).header,datasets(setpos).data,configuration.parameters.channel_labels,'channel_weights',configuration.parameters.channel_weights,'mixed_channel_label',configuration.parameters.mixed_channel_label,'keep_original_channels',configuration.parameters.keep_original_channels);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_pool_channels('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
