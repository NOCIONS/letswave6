function [out_configuration,out_datasets] = LW_correlation(operation,configuration,datasets,update_pointers)
% LW_correlation
% Correlate signals using epochdata
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_correlation';
gui_info.name='Correlate signal with epoch data';
gui_info.description='Correlate signal with epoch data.';
...
gui_info.process_filename_string='corr';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.epochdata_field='';
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Correlation with epochdata.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_correlation(datasets(setpos).header,datasets(setpos).data,'epochdata_field',configuration.parameters.epochdata_field);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_correlation('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
