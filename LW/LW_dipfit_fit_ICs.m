function [out_configuration,out_datasets] = LW_dipfit_fit_ICs(operation,configuration,datasets,update_pointers)
% LW_dipfit_fit_ICs
% DIPFIT : fit dipole(s) onto IC topographies
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_dipfit_fit_ICs';
gui_info.name='dipfit : fit dipole(s) onto IC topographies';
gui_info.description='dipfit : fit dipole(s) onto IC topographies';
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.IC_list=[];
    ...
    case 'process'
        out_datasets=[];
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** DIPFIT : fit dipole(s) onto ICs.',1,0); end;
        %datasets
        out_datasets=datasets;
        for setpos=1:length(datasets);
            ...
            if isempty(ICA_mm);
                if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'No ICA matrix found in the dataset!',1,0); end;
                return;
            end;
            %process
            [dipole_data,message_string]=RLW_dipfit_fit_ICs(datasets(setpos).header,datasets(setpos).data,configuration.parameters.IC_list,ICA_mm,chanlocs,vol,mri,'dipole_model',configuration.parameters.dipole_model,'gridsearch_resolution',configuration.parameters.gridsearch_resolution);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_dipfit_fit_ICs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
