function [out_configuration,out_datasets] = LW_import_TRC(operation,configuration,datasets,update_pointers)
% LW_import_TRC
% Import TRC files
%
% operations : 
...
%gui_info
gui_info.function_name='LW_import_TRC';
gui_info.name='Import Micromed TRC files';
gui_info.description='Import Micromed TRC files.';
...
switch operation
    ...
    case 'process'
        ...
        %configuration
        out_configuration=configuration;
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import TRC data.',1,0); end;
        %inputfiles
        ...
        for filepos=1:length(inputfiles);
            ...
            %process
            [out_datasets(filepos).header out_datasets(filepos).data message_string]=RLW_import_TRC(filename);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_TRC('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
