function [out_configuration,out_datasets] = LW_suppress_artifact_event(operation,configuration,datasets,update_pointers)
% LW_suppress_artifact_event
% Suppress Artifact (event-defined)
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_suppress_artifact_event';
gui_info.name='Suppress artifact (event-defined)';
gui_info.description='Suppress artifact (event-defined).';
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Suppress artifact (event).',1,0); end;
        %datasets
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_suppress_artifact_event('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
