function [out_configuration,out_datasets] = LW_find_peaks(operation,configuration,datasets,update_pointers)
% LW_find_peaks
% find peaks
%
% operations : 
...
%gui_info
gui_info.function_name='LW_find_peaks';
gui_info.name='Find peaks in waveforms';
gui_info.description='Create events from peaks (waveform).';
...
switch operation
    ...
    case 'configure'
        %configuration
        GLW_find_peaks('dummy',configuration,datasets);
        %datasets
        out_configuration=configuration;
        out_datasets=datasets;
end;
