function [out_configuration,out_datasets] = LW_select_epochdata(operation,configuration,datasets,update_pointers)
% LW_select_epochdata
% Select epochs according to epoch data
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_select_epochdata';
gui_info.name='Select epochs according to epoch data';
...
gui_info.process_filename_string='select-epdata';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.fieldname='';
        out_configuration.parameters.logical='==';
        out_configuration.parameters.comparison_value=0;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Select epochs based on epochdata.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_select_epochdata(datasets(setpos).header,datasets(setpos).data,'fieldname',configuration.parameters.fieldname,'logical',configuration.parameters.logical,'comparison_value',configuration.parameters.comparison_value);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_select_epochdata('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
