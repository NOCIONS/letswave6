function [out_configuration,out_datasets] = LW_math_constant(operation,configuration,datasets,update_pointers)
% LW_math_constant
% Math operation using a constant
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_math_constant';
gui_info.name='Math operation using a constant';
gui_info.description='Mathetmatical operation using a constant.';
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.operation='add' %'add' 'subtract' 'multiply' 'divide'
        out_configuration.parameters.constant=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Math operation using a constant.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_math_constant(datasets(setpos).header,datasets(setpos).data,'operation',configuration.parameters.operation,'constant',configuration.parameters.constant);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_math_constant('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
