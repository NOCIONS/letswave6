function [out_configuration,out_datasets] = LW_baseline(operation,configuration,datasets,update_pointers)
% LW_baseline
% Baseline operations
%
% operations : 
...
%gui_info
gui_info.function_name='LW_baseline';
gui_info.name='Baseline correction';
gui_info.description='Baseline correction.';
...
gui_info.process_filename_string='bl';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        if isempty(datasets);
            ...
            out_configuration.parameters.xend=0;
        else
            out_configuration.parameters.xstart=datasets(1).header.xstart;
            out_configuration.parameters.xend=0;
        end;
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_baseline(datasets(setpos).header,datasets(setpos).data,'operation',configuration.parameters.operation,'xstart',configuration.parameters.xstart,'xend',configuration.parameters.xend);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_baseline('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
