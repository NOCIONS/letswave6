function [out_configuration,out_datasets] = LW_ICA_compute(operation,configuration,datasets,update_pointers)
% LW_ICA_compute
% Compute ICA matrix
%
% operations : 
...
%gui_info
gui_info.function_name='LW_ICA_compute';
gui_info.name='Compute ICA matrix';
gui_info.description='Compute ICA mixing/unmixing matrix';
...
switch operation
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [matrix,message_string]=RLW_ICA_compute(datasets(setpos).header,datasets(setpos).data,'num_ICs',configuration.parameters.num_ICs,'ICA_algorithm',configuration.parameters.ICA_algorithm,'ICA_mode',configuration.parameters.ICA_mode,'PICA_percentage',configuration.parameters.PICA_percentage);
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_ICA_compute('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
