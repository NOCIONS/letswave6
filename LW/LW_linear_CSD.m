function [out_configuration,out_datasets] = LW_linear_CSD(operation,configuration,datasets,update_pointers)
% LW_linear_CSD
% Linear CSD
%
% operations : 
...
%gui_info
gui_info.function_name='LW_linear_CSD';
gui_info.name='Compute linear CSD';
gui_info.description='Compute linear Current Source Density (CSD).';
gui_info.parent='csd_menu';
...
gui_info.process_filename_string='lin-csd';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Linear CSD.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_linear_CSD(datasets(setpos).header,datasets(setpos).data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_linear_CSD('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
