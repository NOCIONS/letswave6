function [out_configuration,out_datasets] = LW_hilbert(operation,configuration,datasets,update_pointers)
% LW_hilbert
% Hilbert transform
%
% operations : 
...
%gui_info
gui_info.function_name='LW_hilbert';
gui_info.name='Hilbert transform';
gui_info.description='Compute Hilbert transform.';
...
gui_info.process_filename_string='hilbert';     %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Hilbert transform.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_hilbert(datasets(setpos).header,datasets(setpos).data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_hilbert('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
