function [out_configuration,out_datasets] = LW_sort_events(operation,configuration,datasets,update_pointers)
% LW_sort_events
% Sort epochs according to event latencies
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_sort_events';
gui_info.name='Sort epochs according to event latencies';
gui_info.description='Sort epochs according to event latencies';
...
gui_info.process_filename_string='sort-events';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Sort epochs based on events.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_sort_events(datasets(setpos).header,datasets(setpos).data,'event_code',configuration.parameters.event_code,'sort',configuration.parameters.sort,'discard_empty',configuration.parameters.discard_empty);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_sort_events('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
