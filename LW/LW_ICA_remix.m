function [out_configuration,out_datasets] = LW_ICA_remix(operation,configuration,datasets,update_pointers)
% LW_ICA_remix
% ICA remix signals
%
% operations : 
...
%gui_info
gui_info.function_name='LW_ICA_remix';
gui_info.name='ICA remix signals (ICs > sensors)';
gui_info.description='Remix unmixed signals using previously computed ICA matrix';
...
gui_info.process_filename_string='ica-mm';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.ICA_mm=[];       %if empty, will search history for an existing unmixing matrix (default)
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** ICA remix.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_ICA_remix(datasets(setpos).header,datasets(setpos).data,'ICA_mm',configuration.parameters.ICA_mm,'IC_list',configuration.parameters.IC_list);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_ICA_remix('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
