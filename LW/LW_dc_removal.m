function [out_configuration,out_datasets] = LW_dc_removal(operation,configuration,datasets,update_pointers)
% LW_dc_removal
% DC removal
%
% operations : 
...
%gui_info
gui_info.function_name='LW_dc_removal';
gui_info.name='DC removal and linear detrend';
gui_info.description='Remove DC and/or apply a linear detrend.';
...
gui_info.process_filename_string='dc';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.linear_detrend=0;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** DC Removal and linear detrend.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_dc_removal(datasets(setpos).header,datasets(setpos).data,'linear_detrend',configuration.parameters.linear_detrend);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_dc_removal('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
