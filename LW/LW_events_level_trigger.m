function [out_configuration,out_datasets] = LW_events_level_trigger(operation,configuration,datasets,update_pointers)
% LW_events_level_trigger
...
%gui_info
gui_info.function_name='LW_events_level_trigger';
gui_info.name='Create events from level trigger';
gui_info.description='Create events from level trigger';
...
gui_info.process_filename_string='leveltrg';        %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.threshold=1000;
        out_configuration.parameters.min_isi=1;
        out_configuration.parameters.direction='ascending'; %ascending descending
        out_configuration.parameters.selected_channel=[];
        out_configuration.parameters.event_code='trig';
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Events level trigger.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_events_level_trigger(datasets(setpos).header,datasets(setpos).data,configuration.parameters.selected_channel,'threshold',configuration.parameters.threshold,'min_isi',configuration.parameters.min_isi,'direction',configuration.parameters.direction,'event_code',configuration.parameters.event_code);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_events_level_trigger('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
