function [out_configuration,out_datasets] = LW_hanning_taper(operation,configuration,datasets,update_pointers)
% LW_hanning_taper
% Hanning taper
%
% operations : 
...
%gui_info
gui_info.function_name='LW_hanning_taper';
gui_info.name='Apply Hanning taper';
gui_info.description='Apply Hanning taper.';
...
gui_info.process_filename_string='hanntap';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Hanning taper.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_hanning_taper(datasets(setpos).header,datasets(setpos).data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_hanning_taper('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
