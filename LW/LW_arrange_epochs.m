function [out_configuration,out_datasets] = LW_arrange_epochs(operation,configuration,datasets,update_pointers)
% LW_arrange_epochs
% Rearrange / Delete epochs
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_arrange_epochs';
gui_info.name='Rearrange / Delete epochs';
gui_info.description='Rearrange / Delete epochs';
...
gui_info.process_filename_string='ep-select';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_arrange_epochs(datasets(setpos).header,datasets(setpos).data,configuration.parameters.epoch_idx);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_arrange_epochs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
