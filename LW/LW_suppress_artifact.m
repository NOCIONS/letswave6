function [out_configuration,out_datasets] = LW_suppress_artifact(operation,configuration,datasets,update_pointers)
% LW_suppress_artifact
% Suppress Artifact (latency-defined)
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_suppress_artifact';
gui_info.name='Suppress artifact (latency-defined)';
gui_info.description='Suppress artifact (latency-defined).';
...
gui_info.process_filename_string='art-sup';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Suppress artifact.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_suppress_artifact(datasets(setpos).header,datasets(setpos).data,'xstart',configuration.parameters.xstart,'xend',configuration.parameters.xend);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_suppress_artifact('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
