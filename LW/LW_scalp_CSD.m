function [out_configuration,out_datasets] = LW_scalp_CSD(operation,configuration,datasets,update_pointers)
% LW_scalp_CSD
% Scalp CSD
%
% operations : 
...
%gui_info
gui_info.function_name='LW_scalp_CSD';
gui_info.name='Compute scalp CSD';
gui_info.description='Compute scalp Current Source Density (CSD).';
...
gui_info.process_filename_string='csd';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Scalp CSD.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_scalp_CSD(datasets(setpos).header,datasets(setpos).data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_scalp_CSD('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
