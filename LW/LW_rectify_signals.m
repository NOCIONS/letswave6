function [out_configuration,out_datasets] = LW_rectify_signals(operation,configuration,datasets,update_pointers)
% LW_rectify_signals
% Rectify signals
%
% operations : 
...
%gui_info
gui_info.function_name='LW_rectify_signals';
gui_info.name='Rectify or square signals';
gui_info.description='Rectify or square signals.';
...
gui_info.process_filename_string='rect';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.operation='rectify'; %'rectify','square'
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Rectify/square signals.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            ...
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_rectify_signals(datasets(setpos).header,datasets(setpos).data,'operation',configuration.parameters.operation);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_rectify_signals('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
