function [out_configuration,out_datasets] = LW_wavelet_filter_build(operation,configuration,datasets,update_pointers)
% LW_wavelet_filter_build
% Build wavelet time-frequency filter
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_wavelet_filter_build';
gui_info.name='Build wavelet time-frequency filter (Hu et al.)';
gui_info.description='Build wavelet time-frequency filter.';
...
gui_info.process_filename_string='cwt-template';     %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.start_frequency=1;
        out_configuration.parameters.end_frequency=30;
        out_configuration.parameters.frequency_step=1;
        out_configuration.parameters.threshold=0.85;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Build wavelet filter.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_wavelet_filter_build(datasets(setpos).header,datasets(setpos).data,configuration.parameters.selected_channel,'start_frequency',configuration.parameters.start_frequency,'end_frequency',configuration.parameters.end_frequency,'frequency_step',configuration.parameters.frequency_step,'threshold',configuration.parameters.threshold);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_wavelet_filter_build('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
