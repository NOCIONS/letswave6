function [out_configuration,out_datasets] = LW_wilcoxon(operation,configuration,datasets,update_pointers)
% LW_wilcoxon
...
%gui_info
gui_info.function_name='LW_wilcoxon';
gui_info.name='Compare two datasets (Wilcoxon)';
gui_info.description='Compare two datasets (Wilcoxon).';
...
gui_info.process_filename_string='wilc';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.test_type='signed_rank_test' %'signed_rank_test' 'rank_sum_test' 'sign_test';
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Wilcoxon.',1,0); end;        
        %ref_header, ref_data
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_wilcoxon('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
