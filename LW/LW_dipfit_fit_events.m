function [out_configuration,out_datasets] = LW_dipfit_fit_events(operation,configuration,datasets,update_pointers)
% LW_dipfit_fit_latency
% DIPFIT : fit dipole(s) at event laetencies
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_dipfit_fit_events';
gui_info.name='dipfit : fit dipole(s) at a event latencies';
gui_info.description='dipfit : fit dipole(s) at event latencies';
...
switch operation
    ...
    case 'process'
        out_datasets=[];
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** DIPFIT : fit dipole(s) at event latencies.',1,0); end;
        %datasets
        out_datasets=datasets;
        for setpos=1:length(datasets);
            ...
            %process
            [dipole_data,message_string]=RLW_dipfit_fit_events(datasets(setpos).header,datasets(setpos).data,configuration.parameters.event_code,chanlocs,vol,mri,'dipole_model',configuration.parameters.dipole_model,'gridsearch_resolution',configuration.parameters.gridsearch_resolution);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_dipfit_fit_events('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
