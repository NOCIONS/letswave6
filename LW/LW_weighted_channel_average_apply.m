function [out_configuration,out_datasets] = LW_weighted_channel_average_apply(operation,configuration,datasets,update_pointers)
% LW_weighted_channel_average_apply
...
%gui_info
gui_info.function_name='LW_weighted_channel_average_apply';
gui_info.name='Weighted channel average (apply template)';
gui_info.description='Weighted channel average (apply template).';
...
gui_info.process_filename_string='chanavg';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_weighted_channel_average_apply(datasets(setpos).header,datasets(setpos).data,'template_labels',configuration.parameters.template_labels,'template_weights',configuration.parameters.template_weights);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_weighted_channel_average_apply('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
