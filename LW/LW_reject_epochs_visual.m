function [out_configuration,out_datasets] = LW_reject_epochs_visual(operation,configuration,datasets,update_pointers)
% LW_reject_epochs_visual
% Reject epochs based on visual inspection
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_reject_epochs_visual';
gui_info.name='Reject epochs (visual inspection)';
gui_info.description='Reject epochs based on visual inspection';
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.rejected_epochs=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Reject epochs.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_reject_epochs(datasets(setpos).header,datasets(setpos).data,configuration.parameters.rejected_epochs);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_reject_epochs_visual('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
