function [out_configuration,out_datasets] = LW_import_BDF(operation,configuration,datasets,update_pointers)
% LW_import_BDF
% Import BDF files
%
% operations : 
...
%gui_info
gui_info.function_name='LW_import_BDF';
gui_info.name='Import Biosemi BDF files';
gui_info.description='Import Biosemi BDF data files.';
...
switch operation
    ...
    case 'process'
        ...
        %configuration
        out_configuration=configuration;
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import BDF data.',1,0); end;
        %inputfiles
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_BDF('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
