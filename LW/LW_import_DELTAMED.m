function [out_configuration,out_datasets] = LW_import_DELTAMED(operation,configuration,datasets,update_pointers)
% LW_import_DELTAMED
% Import Deltamed ASC/BIN files
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_import_DELTAMED';
gui_info.name='Import Deltamed BIN/ASC files';
gui_info.description='Import Deltamed BIN/ASC files.';
...
switch operation
    ...
    case 'process'
        ...
        %configuration
        out_configuration=configuration;
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import DELTAMED data.',1,0); end;
        %inputfiles
        ...
        for filepos=1:length(inputfiles);
            ...
            %process
            [out_datasets(filepos).header out_datasets(filepos).data message_string]=RLW_import_DELTAMED(filename);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_DELTAMED('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
