function [out_configuration,out_datasets] = LW_ICA_unmix(operation,configuration,datasets,update_pointers)
% LW_ICA_unmix
% ICA unmix signals
%
% operations : 
...
%gui_info
gui_info.function_name='LW_ICA_unmix';
gui_info.name='ICA unmix signals (sensors > ICs)';
gui_info.description='Unmix signals using previously computed ICA matrix';
...
gui_info.process_filename_string='ica-um';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.old_chanlocs=[]; %to be used when remixing ICs
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** ICA unmix.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string,original_chanlocs]=RLW_ICA_unmix(datasets(setpos).header,datasets(setpos).data,'ICA_um',configuration.parameters.ICA_um);
            %message_string
            ...
            %store old chanlocs
            configuration.parameters.old_chanlocs=original_chanlocs;
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_ICA_unmix('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
