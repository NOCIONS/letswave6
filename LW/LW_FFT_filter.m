function [out_configuration,out_datasets] = LW_FFT_filter(operation,configuration,datasets,update_pointers)
% LW_FFT_filter
% FFT filter
%
% operations : 
...
%gui_info
gui_info.function_name='LW_FFT_filter';
gui_info.name='FFT filter';
gui_info.description='Apply a low-pass, high-pass, band-pass or notch FFT filter';
...
gui_info.process_filename_string='filt-fft';    %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.low_width=0.25;
        out_configuration.parameters.high_width=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** FFT filter.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_FFT_filter(datasets(setpos).header,datasets(setpos).data,'filter_type',configuration.parameters.filter_type,'low_cutoff',configuration.parameters.low_cutoff,'high_cutoff',configuration.parameters.high_cutoff,'low_width',configuration.parameters.low_width,'high_width',configuration.parameters.high_width);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_FFT_filter('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
