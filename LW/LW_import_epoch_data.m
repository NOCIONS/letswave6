function [out_configuration,out_datasets] = LW_import_epoch_data(operation,configuration,datasets,update_pointers)
% LW_import_epoch_data
% import epoch data
%
% operations : 
% 'gui_info'
...
%gui_info
gui_info.function_name='LW_import_epoch_data';
gui_info.name='Add or edit epoch data';
gui_info.description='ADd or edit epoch data.';
...
gui_info.process_filename_string='epochdata';    %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.epoch_data=[]; % epoch data will be replaced by this epoch data
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import epoch data.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_import_epoch_data(datasets(setpos).header,configuration.parameters.epoch_data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_epoch_data('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
