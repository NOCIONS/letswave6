function [out_configuration,out_datasets] = LW_edit_electrodes(operation,configuration,datasets,update_pointers)
% LW_edit_electrodes
% edit electrodes (scalp coordinates)
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_edit_electrodes';
gui_info.name='Edit electrode coordinates';
gui_info.description='Edit electrode coordinates (scalp EEG).';
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Edit electrodes.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_edit_electrodes(datasets(setpos).header,configuration.parameters.chanlocs);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_edit_electrodes('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
