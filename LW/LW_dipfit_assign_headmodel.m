function [out_configuration,out_datasets] = LW_dipfit_assign_headmodel(operation,configuration,datasets,update_pointers)
% LW_dipfit_assign_headmodel
% DIPFIT : assign head model
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_dipfit_assign_headmodel';
gui_info.name='dipfit : load 3D head model';
gui_info.description='dipfit : load 3D head model.';
...
gui_info.process_filename_string='hdm';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.hdm_filename=[];
        out_configuration.parameters.vol=[]; %process
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        out_datasets=[];
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** DIPFIT : assign headmodel.',1,0); end;
        %datasets
        out_datasets=datasets;
        for setpos=1:length(datasets);
            %process
            [vol,message_string]=RLW_dipfit_assign_headmodel(configuration.parameters.hdm_filename);
            %message_string
            ...
            configuration.parameters.vol=vol;
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_dipfit_assign_headmodel('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
