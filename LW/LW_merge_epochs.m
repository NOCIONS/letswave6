function [out_configuration,out_datasets] = LW_merge_epochs(operation,configuration,datasets,update_pointers)
% LW_merge_epochs
...
%gui_info
gui_info.function_name='LW_merge_epochs';
gui_info.name='Merge epochs of multiple datasets';
gui_info.description='Merge epochs of multiple datasets.';
...
gui_info.process_filename_string='merged_epochs';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Merge epochs of different datasets.',1,0); end;
        %process
        [out_datasets.header,out_datasets.data,message_string]=RLW_merge_epochs(datasets,configuration.parameters.merge_idx);
        %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_merge_epochs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
