function [out_configuration,out_datasets] = LW_headmodel_compute(operation,configuration,datasets,update_pointers)
% LW_headmodel_compute
% Compute headmodel splinefile
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_headmodel_compute';
gui_info.name='Compute spline headmodel';
gui_info.description='Compute spline model for headplot.';
...
gui_info.process_filename_string='headmod';     %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.spl_filename=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        out_datasets=datasets;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Compute head model.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [filename,message_string]=RLW_headmodel_compute(datasets(setpos).header);
            %message_string
            ...
            configuration.parameters.spl_filename=filename;
            %configuration
            out_configuration=configuration;
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_headmodel_compute('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
