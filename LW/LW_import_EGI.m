function [out_configuration,out_datasets] = LW_import_EGI(operation,configuration,datasets,update_pointers)
% LW_import_EGI
% Import EGI files
%
% operations : 
...
%gui_info
gui_info.function_name='LW_import_EGI';
gui_info.name='Import Netstation EGI files';
gui_info.description='Import EGI data files.';
...
switch operation
    ...
    case 'process'
        ...
        %configuration
        out_configuration=configuration;
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import EGI data.',1,0); end;
        %inputfiles
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_EGI('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
