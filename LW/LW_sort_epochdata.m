function [out_configuration,out_datasets] = LW_sort_epochdata(operation,configuration,datasets,update_pointers)
% LW_sort_epochdata
% Sort epochs according to epoch data
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_sort_epochdata';
gui_info.name='Sort epochs according to epoch data';
gui_info.description='Sort epochs according to epoch data';
...
gui_info.process_filename_string='sort-epdata';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.sort='ascend';
        out_configuration.parameters.discard_empty=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Sort epochs based on epochdata.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_sort_epochdata(datasets(setpos).header,datasets(setpos).data,'fieldname',configuration.parameters.fieldname,'sort',configuration.parameters.sort,'discard_empty',configuration.parameters.discard_empty);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_sort_epochdata('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
