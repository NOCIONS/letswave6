function [out_configuration,out_datasets] = LW_concatenate_epochs(operation,configuration,datasets,update_pointers)
% LW_concatenate_epochs
% Concatenate epochs
%
% operations : 
% 'gui_info'
...
%gui_info
gui_info.function_name='LW_concatenate_epochs';
gui_info.name='Concatenate epochs';
gui_info.description='Concatenate epochs';
...
gui_info.process_filename_string='concat';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.epoch_idx=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Arrange/Delete epochs.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_concatenate_epochs(datasets(setpos).header,datasets(setpos).data,configuration.parameters.epoch_idx);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_concatenate_epochs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
