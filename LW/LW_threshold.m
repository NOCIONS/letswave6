function [out_configuration,out_datasets] = LW_threshold(operation,configuration,datasets,update_pointers)
% LW_threshold
% Threshold signals
%
% operations : 
...
%gui_info
gui_info.function_name='LW_threshold';
gui_info.name='Threshold signals';
gui_info.description='Threshold signals.';
...
gui_info.process_filename_string='threshold';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.threshold_value=0;
        out_configuration.parameters.threshold_criterion='<'; %'<' '<=' '>' '>=' '=='
        out_configuration.parameters.consecutivity_criterion=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Threshold.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_threshold(datasets(setpos).header,datasets(setpos).data,'threshold_value',configuration.parameters.threshold_value,'threshold_criterion',configuration.parameters.threshold_criterion,'consecutivity_criterion',configuration.parameters.consecutivity_criterion);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_threshold('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
