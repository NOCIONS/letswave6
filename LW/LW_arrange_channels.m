function [out_configuration,out_datasets] = LW_arrange_channels(operation,configuration,datasets,update_pointers)
% LW_arrange_channels
% Rearrange / Delete channels
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_arrange_channels';
gui_info.name='Rearrange / Delete channels';
gui_info.description='Rearrange / Delete channels';
...
gui_info.process_filename_string='chan-select'; %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Arrange/Delete channels.',1,0); end;
        %datasets
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_arrange_channels('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
