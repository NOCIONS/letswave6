function [out_configuration,out_datasets] = LW_downsample(operation,configuration,datasets,update_pointers)
% LW_downsample
% Downsample (ratio)
%
% operations : 
...
%gui_info
gui_info.function_name='LW_downsample';
gui_info.name='Downsample signals (integer ratio)';
gui_info.description='Downsample signals (using an integer ratio)';
...
gui_info.process_filename_string='ds';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.x_downsample_ratio=1;
        out_configuration.parameters.y_downsample_ratio=1;
        out_configuration.parameters.z_downsample_ratio=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Downsample.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_downsample(datasets(setpos).header,datasets(setpos).data,'x_downsample_ratio',configuration.parameters.x_downsample_ratio,'y_downsample_ratio',configuration.parameters.y_downsample_ratio,'z_downsample_ratio',configuration.parameters.z_downsample_ratio);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_downsample('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
