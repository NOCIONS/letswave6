function [out_configuration,out_datasets] = LW_select_events(operation,configuration,datasets,update_pointers)
% LW_select_events
% Select epochs according to event latencies
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_select_events';
gui_info.name='Select epochs according to event latencies';
gui_info.description='Select epochs according to epoch data';
...
gui_info.process_filename_string='select-events';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration.parameters.minimum_latency=0;
        out_configuration.parameters.maximum_latency=1;
        out_configuration.parameters.check_minimum_latency=1;
        out_configuration.parameters.check_maximum_latency=1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Select epochs based on events.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_select_events(datasets(setpos).header,datasets(setpos).data,'event_code',configuration.parameters.event_code,'minimum_latency',configuration.parameters.minimum_latency,'maximum_latency',configuration.parameters.maximum_latency,'check_minimum_latency',configuration.parameters.check_minimum_latency,'check_maximum_latency',configuration.parameters.check_maximum_latency);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_select_events('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
