function [out_configuration,out_datasets] = LW_rereference_advanced(operation,configuration,datasets,update_pointers)
% LW_rereference_advanced
...
%gui_info
gui_info.function_name='LW_rereference_advanced';
gui_info.name='Rereference (custom montage)';
gui_info.description='Calculate and apply a new reference (custom montage).';
gui_info.parent='preprocess_menu';
...
gui_info.process_filename_string='mtg';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.active_channel_labels={};
        out_configuration.parameters.reference_channel_labels={};
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Rereference (custom montage).',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_rereference_advanced(datasets(setpos).header,datasets(setpos).data,configuration.parameters.active_channel_labels,configuration.parameters.reference_channel_labels);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_rereference_advanced('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
