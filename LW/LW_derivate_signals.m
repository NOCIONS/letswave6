function [out_configuration,out_datasets] = LW_derivate_signals(operation,configuration,datasets,update_pointers)
% LW_derivate_signals
% Derivate signals
%
% operations : 
...
%gui_info
gui_info.function_name='LW_derivate_signals';
gui_info.name='Compute signal derivative';
gui_info.description='Compute signal derivative.';
...
gui_info.process_filename_string='deriv';          %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.operation=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Derivate signals.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_derivate_signals(datasets(setpos).header,datasets(setpos).data);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_derivate_signals('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
