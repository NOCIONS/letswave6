function [out_configuration,out_datasets] = LW_flip_electrodes(operation,configuration,datasets,update_pointers)
% LW_flip_electrodes
% flip electrodes
%
% operations : 
...
%gui_info
gui_info.function_name='LW_flip_electrodes';
gui_info.name='Flip left/right electrodes';
gui_info.description='Flip left/right electrodes.';
...
gui_info.process_filename_string='chanflip';  %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.chan_labels={}; %col1=old; col2=new
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,out_datasets(setpos).data,message_string]=RLW_flip_electrodes(datasets(setpos).header,datasets(setpos).data,configuration.parameters.chan_labels);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_flip_electrodes('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
