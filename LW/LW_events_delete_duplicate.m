function [out_configuration,out_datasets] = LW_events_delete_duplicate(operation,configuration,datasets,update_pointers)
% LW_events_delete_duplicate
% Events delete duplicate
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_events_delete_duplicate';
gui_info.name='Delete duplicate events';
gui_info.description='Delete duplicate events';
...
gui_info.process_filename_string='del-dup';        %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.exact_latencies=1;
        out_configuration.parameters.tolerance=0.1;
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Delete duplicate events.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_events_delete_duplicate(datasets(setpos).header,'exact_latencies',configuration.parameters.exact_latencies,'tolerance',configuration.parameters.tolerance);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_events_delete_duplicate('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
