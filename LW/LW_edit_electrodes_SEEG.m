function [out_configuration,out_datasets] = LW_edit_electrodes_SEEG(operation,configuration,datasets,update_pointers)
% LW_edit_electrodes_SEEG
% edit electrodes (SEEG)
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_edit_electrodes_SEEG';
gui_info.name='Edit electrode coordinates (SEEG)';
gui_info.description='Edit electrode coordinates (SEEG).';
...
gui_info.process_filename_string='seeglocs';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.list_labels={};
        out_configuration.parameters.list_X=[];
        out_configuration.parameters.list_Y=[];
        out_configuration.parameters.list_Z=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Edit electrodes (SEEG).',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [out_datasets(setpos).header,message_string]=RLW_edit_electrodes_SEEG(datasets(setpos).header,configuration.parameters.list_labels,configuration.parameters.list_X,configuration.parameters.list_Y,configuration.parameters.list_Z);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_edit_electrodes_SEEG('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
