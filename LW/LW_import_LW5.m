function [out_configuration,out_datasets] = LW_import_LW5(operation,configuration,datasets,update_pointers)
% LW_import_LW5
% Import Letswave 5 files
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_import_LW5';
gui_info.name='Import Letswave 5 files';
gui_info.description='Import Letswave 5 files.';
...
switch operation
    ...
    case 'process'
        ...
        %configuration
        out_configuration=configuration;
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Import LW5 data.',1,0); end;
        %inputfiles
        ...
        for filepos=1:length(inputfiles);
            ...
            %process
            [out_datasets(filepos).header out_datasets(filepos).data message_string]=RLW_import_LW5(filename);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_import_LW5('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
