function [out_configuration,out_datasets] = LW_dipfit_assign_chanlocs(operation,configuration,datasets,update_pointers)
% LW_dipfit_assign_chanlocs
% DIPFIT : assign channel coordinates
%
% operations : 
% 'gui_info'
% 'default'
...
%gui_info
gui_info.function_name='LW_dipfit_assign_chanlocs';
gui_info.name='dipfit : load channel coordinates';
gui_info.description='dipfit : load channel locations.';
...
gui_info.process_filename_string='chanlocs';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.chanloc_filename=[];
        out_configuration.parameters.chanlocs=[]; %process
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        out_datasets=[];
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** DIPFIT : assign channel locations.',1,0); end;
        %datasets
        out_datasets=datasets;
        for setpos=1:length(datasets);
            %process
            [chanlocs,message_string]=RLW_dipfit_assign_chanlocs(configuration.parameters.chanloc_filename);
            %message_string
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_dipfit_assign_chanlocs('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
