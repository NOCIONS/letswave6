function [out_configuration,out_datasets] = LW_PCA_compute(operation,configuration,datasets,update_pointers)
% LW_PCA_compute
% Compute PCA matrix
%
% operations : 
...
%gui_info
gui_info.function_name='LW_PCA_compute';
gui_info.name='Compute PCA matrix';
gui_info.description='Compute PCA mixing/unmixing matrix';
...
gui_info.process_filename_string='PCA';         %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** Compute PCA decomposition.',1,0); end;
        %datasets
        for setpos=1:length(datasets);
            %process
            [matrix,message_string]=RLW_PCA_compute(datasets(setpos).header,datasets(setpos).data);
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_PCA_compute('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
