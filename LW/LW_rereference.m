function [out_configuration,out_datasets] = LW_rereference(operation,configuration,datasets,update_pointers)
% LW_rereference
% rereference
%
% operations : 
...
%gui_info
gui_info.function_name='LW_rereference';
gui_info.name='Rereference';
gui_info.description='Calculate and apply a new reference.';
...
gui_info.process_filename_string='reref';       %default filename suffix (or filename (if 'unique'))
...
switch operation
    ...
    case 'default'
        ...
        out_configuration=configuration;
        out_configuration.gui_info=gui_info;
        out_configuration.parameters.apply_list=[];
        out_configuration.parameters.reference_list=[];
        %datasets
        out_datasets=datasets;
    ...
    case 'process'
        ...
        out_configuration=configuration;
        %handles feedback
        if isempty(update_pointers) else update_pointers.function(update_pointers.handles,'*** New reference.',1,0); end;
        %datasets
    ...
    case 'configure'
        %configuration
        [a out_configuration]=GLW_rereference('dummy',configuration,datasets);
        %datasets
        out_datasets=datasets;
end;
