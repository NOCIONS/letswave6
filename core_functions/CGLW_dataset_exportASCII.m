function varargout = CGLW_dataset_exportASCII(varargin)
% CGLW_DATASET_EXPORTASCII MATLAB code for CGLW_dataset_exportASCII.fig
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @CGLW_dataset_exportASCII_OpeningFcn, ...
                   'gui_OutputFcn',  @CGLW_dataset_exportASCII_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CGLW_dataset_exportASCII is made visible.
function CGLW_dataset_exportASCII_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure1
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CGLW_dataset_exportASCII (see VARARGIN)
...
function varargout = CGLW_dataset_exportASCII_OutputFcn(hObject, eventdata, handles) 
...
for i=1:length(st);
    ...
    if header.datasize(3)>1;
        disp('Cannot export datasets with more than one index');
        disp('Will only export the first index');
    end;
    if header.datasize(4)>1;
        disp('Cannot export datasets with more than one Z');
        disp('Will only export the first Z line');
        return;
    end;
    if header.datasize(5)>1;
        disp('Cannot export datasets with more than one Y');
        disp('Will only export the Y line');
    end;
    tp=squeeze(data(1,:,1,1,1,:));
    if header.datasize(1)>1;
        for j=2:header.datasize(1);
            tp=[tp squeeze(data(j,:,1,1,1,:))];
        end;
    end;
    %export
