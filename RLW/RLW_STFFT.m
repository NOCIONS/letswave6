function [out_header,out_data,message_string,time_header]=RLW_STFFT(header,data,varargin);
%RLW_STFFT
%
%ST-FFT
%
...
if isempty(varargin);
    ...
    %sliding_step
    a=find(strcmpi(varargin,'sliding_step'));
    if isempty(a);
    else
        sliding_step=varargin{a+1};
    end;
    %low_frequency
...
%ajust out_header ystep
out_header.ystep=freq_step;

%dx_hanning_width
dx_hanning_width=fix(hanning_width/header.xstep);

%dxstep > noverlap
noverlap=dx_hanning_width-sliding_step;
message_string{end+1}=['Window : ',num2str(dx_hanning_width),' NOverlap : ',num2str(noverlap)];

%k
k=fix((header.datasize(6)-noverlap)/(dx_hanning_width-noverlap));

%adjust out_header datasize
out_header.datasize(6)=k;
out_header.datasize(5)=num_frequency_lines;

%adjust out_header xstart and xstep
message_string{end+1}='Preparing STFFT...';
[S,F,T]=spectrogram(squeeze(data(1,1,1,1,1,:)),dx_hanning_width,noverlap,frequencies,1/header.xstep);
out_header.xstart=header.xstart+T(1);
out_header.xstep=T(2)-T(1);

%adjust out_header datasize if average
if average_epochs==1;
...
%prepare out_array
out_array=zeros(out_header.datasize(5),out_header.datasize(6));
    ...
    for indexpos=1:header.datasize(3);
        ...
        for epochpos=1:header.datasize(1);
            ...
            switch postprocess
                ...
                case 'angle'
                    out_array(:,:)=angle(spectrogram(squeeze(data(epochpos,channelpos,indexpos,dz,dy,:)),dx_hanning_width,noverlap,frequencies,1/header.xstep));
            end;
            %average?
            if average_epochs==1;
                %out_data is sum of out_array
                out_data(1,channelpos,indexpos,dz,:,:)=squeeze(out_data(1,channelpos,indexpos,dz,:,:))+out_array;
            else
                out_data(epochpos,channelpos,indexpos,dz,:,:)=out_array;
            end;
        end;
    end;
end;
