function [out_datasets,message_string]=RLW_segmentation2(header,data,event_labels,varargin);
%RLW_segmentation2
%
%segment data (one dataset per event code)
%
%%event_labels={$};
%
%varargin
...
%init setpos (for each dataset, output can be multiple datasets
setpos=1;
...
for event_labels_pos=1:length(event_labels);
    ...
    if isempty(event_idx);
        ...
        %add event_code to header.name
        out_header.name=[event_code ' ' header.name];
        %prepare out_data
    ...
    %check that out_data is not empty
    if isempty(out_data);
    else
