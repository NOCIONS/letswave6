function [regressor_header,regressor_data,peak_estimates,message_string]=RLW_single_trial_MLR_distortion(header,data,varargin);
...
regressor_header.datasize=[1 1 1+(length(ST.regressor)*3) 1 1 length(ST.t_idx)];
...
for i=1:length(ST.regressor);
    regressor_header.indexlabels{2+((i-1)*3)}=['peak' num2str(i) '_1'];
    regressor_header.indexlabels{3+((i-1)*3)}=['peak' num2str(i) '_2'];
    regressor_header.indexlabels{4+((i-1)*3)}=['peak' num2str(i) '_3'];
end;
...
for i=1:length(ST.regressor);
    regressor_data(1,1,2+((i-1)*3),1,1,:)=ST.regressor{i}(:,1);
    regressor_data(1,1,3+((i-1)*3),1,1,:)=ST.regressor{i}(:,2);
    regressor_data(1,1,4+((i-1)*3),1,1,:)=ST.regressor{i}(:,3);
end;
