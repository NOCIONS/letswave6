            ...
            switch test_type
                case 'signed_rank_test'
                    for dx=1:header1.datasize(6);
                        ...
                        [actual_tres_pvalue(dy,dx),H,STATS]=signrank(tpleft,tpright,'alpha',alpha,'tail',tails, 'method','approximate');
                ...
                case 'rank_sum_test'
                    for dx=1:header1.datasize(6);
                        tpleft=squeeze(data1(:,chanpos,indexpos,dz,dy,dx));
                        tpright=squeeze(data2(:,chanpos,indexpos,dz,dy,dx));
                        [actual_tres_pvalue(dy,dx),H,STATS]=ranksum(tpleft,tpright,'alpha',alpha,'tail',tails, 'method','approximate');
                        actual_tres_Zvalue(dy,dx)=STATS.zval;
                    end;
                case 'signed_test'
                    for dx=1:header1.datasize(6);
                        tpleft=squeeze(data1(:,chanpos,indexpos,dz,dy,dx));
                        tpright=squeeze(data2(:,chanpos,indexpos,dz,dy,dx));
                        [actual_tres_pvalue(dy,dx),H,STATS]=signtest(tpleft,tpright,'alpha',alpha,'tail',tails, 'method','approximate');
                        actual_tres_Zvalue(dy,dx)=STATS.zval;
                    end;
            end;
        end;
                    ...
                    switch test_type
                        case 'signed_rank_test'
                            for dx=1:size(data1,6);
                                ...
                                [p,H,STATS]=signrank(tpleft,tpright,'alpha',alpha,'tail',tails, 'method','approximate');
