function [wav1 wav2]=LW_fastwavelet_mother(type,periods,stdev,mothersize)
%Calculate wavelet

if strcmpi(type,'morlet');
    idx=0:1:mothersize-1;
    idx=(idx-(mothersize/2))/(mothersize-1);
...
if strcmpi(type,'hanning');
    id=0:1:mothersize-1;
    idx=(id-(mothersize/2))/(mothersize-1);
    tp1=sin(idx*2*pi*periods);
    tp2=cos(idx*2*pi*periods);
    tp3=0.5*(1-cos((2*pi*id)/mothersize));
    wav1=tp1.*tp3;
    wav2=tp2.*tp3;
%     pivar:=pi;
%     {Generate a Morlet Wavelet wav1 = real, wav2 = imag}
