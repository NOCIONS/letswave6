function [event] = ft_read_event_EDFMAX(filename, varargin)
...
%   'headerformat'   string
%   'eventformat'    string
%   'header'         header structure, see ft_read_header_EDFMAX
...
%
% The list of supported file formats can be found in ft_read_header_EDFMAX.
%
...
%
% See also ft_read_header_EDFMAX, FT_READ_DATA, FT_WRITE_EVENT, FT_FILTER_EVENT
...
if iscell(filename)
  ...
  for i=1:numel(filename)
    ...
    event{i} = ft_read_event_EDFMAX(filename{i}, varargin{:});
...
checkmaxfilter   = ft_getopt(varargin, 'checkmaxfilter');            % will be passed to ft_read_header_EDFMAX
...
switch eventformat
  ...
  case {'4d' '4d_pdf', '4d_m4d', '4d_xyz'}
    if isempty(hdr)
      hdr = ft_read_header_EDFMAX(filename, 'headerformat', eventformat);
    end
  ...
  case {'biosemi_bdf', 'bham_bdf'}
    ...
    sdata = ft_read_data_EDFMAX(filename, 'header', hdr, 'dataformat', dataformat, 'begsample', begsample, 'endsample', endsample, 'chanindx', schan);
  ...
  case {'ctf_ds', 'ctf_meg4', 'ctf_res4', 'ctf_old'}
    ...
    % read the header, required to determine the stimulus channels and trial specification
    if isempty(hdr)
      hdr = ft_read_header_EDFMAX(headerfile, 'headerformat', headerformat);
    end
  ...
  case 'dataq_wdq'
    if isempty(hdr)
      hdr     = ft_read_header_EDFMAX(filename, 'headerformat', 'dataq_wdq');
    end
  ...
  case {'eep_cnt' 'eep_trg'}
    ...
    if isempty(hdr)
      if exist(cntfile, 'file')
        hdr = ft_read_header_EDFMAX(cntfile);
      else
  ...
  case 'egi_sbin'
    ...
    if isempty(hdr)
      hdr = ft_read_header_EDFMAX(filename,'headerformat','egi_sbin');
    end
  ...
  case {'neuromag_eve'}
    ...
    if isempty(hdr)
      hdr = ft_read_header_EDFMAX(fiffile, 'headerformat', 'neuromag_mne');
    end
  ...
  case {'neuromag_fif' 'neuromag_mne' 'neuromag_mex'}
    ...
    if isempty(hdr)
      hdr = ft_read_header_EDFMAX(filename, 'headerformat', headerformat, 'checkmaxfilter', checkmaxfilter);
    end
    ...
    if exist(evefile, 'file')
      eve = ft_read_event_EDFMAX(evefile, 'header', hdr);
      if ~isempty(eve)
  ...
  case 'neuroomega_mat'
    ...
    hdr = ft_read_header_EDFMAX(filename, 'headerformat', eventformat, 'chantype', 'chaninfo');
  ...
  case 'neuralynx_cds'
    ...
    if hasttl
      ...
      event = ft_read_event_EDFMAX(eventfile);
