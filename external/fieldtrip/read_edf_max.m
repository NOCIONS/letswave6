function [dat] = read_edf_max(filename, hdr, begsample, endsample, chanindx)
...
if needhdr
  ...
  elseif all(EDF.SampleRate(1:end-1)==EDF.SampleRate(1))
    ...
    % MODIFIED : select the sample rate that are max
    a=max(EDF.SampleRate);
    chansel=find(EDF.SampleRate==a);
