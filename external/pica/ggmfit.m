...
if(method == 'ggm')
   ...
   if((sum(idx2)>0)&(pis(2)>0.001))
      pygx(2,idx2) = gammapdf(mus(2).*mus(2)/sig(2),mus(2)/sig(2)+0.00001,dat2);
   else
      pygx(2,idx2) = 0.0;
   end;
   if(nummix>2)
      ...
      pygx(3,idx3) = gammapdf(mus(3).*mus(3)/sig(3),-mus(3)/sig(3)-0.00001,dat3);%
   end;
else
...
if(method == 'ggm'); 
   ...
   if(pis(2)>0.0001)
      grot1(2,aidx2) = pis(2)*gammapdf(mus(2).*mus(2)/sig(2),mus(2)/sig(2)+0.001,dal2');
   else
      grot1(2,aidx2) = 0.0;
      end;
   if(nummix>2)
      ...
      grot1(3,aidx3) = pis(3)*gammapdf(mus(3).*mus(3)/sig(3),-mus(3)/sig(3)+0.001,dal3');
